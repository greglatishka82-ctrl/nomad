...
class _SupportChatScreenState extends ConsumerState<SupportChatScreen> {
  ...
  void _playNotificationSound() async {
    try {
      await _audioPlayer.play(AssetSource('sounds/support.mp3'));
    } catch (e) {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      await dio.post('/api/mobile/support/messages',
          data: {'text': text});
      await ref.read(supportMessagesProvider.notifier).refresh();
        ...
        children: [
          Expanded(
            child: messagesAsync.when(
              ...
              data: (messages) {
                // Воспроизводим звук при новом сообщении от админа
                if (messages.isNotEmpty && _previousMessageCount > 0 && messages.length > _previousMessageCount) {
