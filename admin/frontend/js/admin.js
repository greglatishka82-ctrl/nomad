...
async function fetchWithTimeout(input, init = {}, timeoutMs = INITIAL_SESSION_TIMEOUT_MS) {
...
let instructorSaveInProgress = false;
const adminMutationInFlight = new Map();
const applicationAlertAudio = new Audio('/bebe.mp3');
...
async function heartbeat() {
    // Do not stack a second expensive snapshot while a slow network request
    // from the previous heartbeat is still running.
...
async function buildOfflineSlots(bookingDate, serviceType, transmission, selectedInstructorId) {
    ...
    const activeBookings = allBookings.filter(booking => {
        const statuses = booking.is_mobile
            ? ['pending', 'planned', 'confirmed', 'in_progress']
    ...
    for (let start = startHour * 60; start <= endHour * 60; start += duration) {
        ...
        const peakUsage = bookings => {
            const events = [];
            for (const booking of bookings) {
                const busyStart = Math.max(start, minutesFromTime(booking.start_time));
                const busyEnd = Math.min(end, bookingEnd(booking));
                if (busyStart < busyEnd) events.push([busyStart, 1], [busyEnd, -1]);
            }
        ...
        const bookedAtLocation = peakUsage(activeBookings.filter(booking => booking.location === location));
        const bookedCompatibleVehicles = peakUsage(activeBookings.filter(booking =>
            booking.transmission === transmission
        ));
        const bookedExamSensors = peakUsage(activeBookings.filter(booking =>
        ...
        slots.push({
            ...
            booked_count: bookedCompatibleVehicles, capacity: vehicleCapacity,
            ...
            is_free: availableIds.length > 0
                && (serviceType !== 'exam' || transmission === 'automatic')
                && (serviceType !== 'exam' || bookedExamSensors < examSensorCapacity)
...
async function playNewApplicationAlert() {
    if (!applicationSoundEnabled) return;
    if (_applicationAlertPlayback) return _applicationAlertPlayback;
    ...
    const playback = (async () => {
        for (let attempt = 0; attempt < 3 && playbackId === _applicationAlertPlaybackId && applicationSoundEnabled; attempt += 1) {
            try {
                ...
                // Браузер может блокировать звук до первого действия администратора.
                break;
            }
...
async function pollNotificationCounts() {
    try {
        ...
        _prevCounts = counts;
        _notificationCountsInitialized = true;
        updateNavBadge('clients', counts.new_clients);
        updateNavBadge('support', counts.unread_support);
        // Заявка не становится обработанной от простого открытия вкладки.
        // Бейдж снимается только после подтверждения/отклонения всех заявок.
...
const pageTitles = {
    ...
    analytics: 'Аналитика', packages: 'Скидки', support: 'Поддержка',
...
async function navigateTo(page) {
    ...
    if (viewedPath[page]) {
        try {
            const viewed = await apiPost(viewedPath[page], {});
            // Офлайн-ответ не записал watermark на сервер. Не изображаем
            // успешное чтение: иначе следующий online polling вернёт бейдж.
            if (!viewed?.offline) {
                _badgeViewVersion += 1;
    ...
    // Load data
    const loaders = { dashboard: loadDashboard, bookings: loadBookings, instructors: loadInstructors, vehicles: loadVehicles, clients: loadClients, analytics: loadAnalytics, packages: loadPackages, support: loadSupport, faq: loadFaq, notifications: loadNotifications, audit: loadAudit, settings: loadSettings };
    if (loaders[page]) {
...
// Individual actions may also refresh their own list immediately; this small
// debounce covers the remaining actions without issuing a reload per request
// when one form saves several related records (client + package + certificate).
let adminDataRefreshTimer = null;
function scheduleAdminDataRefresh(path) {
    // These POST requests only acknowledge a viewed badge; they do not change
    // business data and must not restart a page load during navigation.
    if (/\/(bookings|notifications|support|clients)\/mark-viewed$/.test(path)) return;
    clearTimeout(adminDataRefreshTimer);
    adminDataRefreshTimer = setTimeout(() => {
        const activePage = document.querySelector('.page.active')?.id?.replace(/^page-/, '');
        const loaders = {
            dashboard: loadDashboard, bookings: loadBookings, instructors: loadInstructors, vehicles: loadVehicles,
            clients: loadClients, analytics: loadAnalytics, packages: loadPackages,
            support: loadSupport, faq: loadFaq, notifications: loadNotifications, audit: loadAudit,
            settings: loadSettings,
        };
        loaders[activePage]?.().catch((error) => {
            console.error('Не удалось автоматически обновить данные', error);
        });
    }, 350);
}
...
async function apiPostOnce(path, data, operationId) {
    // A create can reach the server successfully while its response is lost.
    // Keep one stable key for the online attempt and its queued retry.
    if (offlineReplayInProgress) await waitForOfflineReplay();
    if (isAdminOffline) {
        if (path.includes('/mark-viewed')) return { ok: true, offline: true };
    ...
    try {
        const headers = {'Content-Type': 'application/json', 'X-Idempotency-Key': operationId};
        const res = await fetch(`${API}${path}`, { method: 'POST', headers, body: JSON.stringify(data), credentials: 'include' });
        if (res.status === 401) { showLogin('Сессия завершена. Войдите снова.'); throw new Error('Сессия завершена. Войдите снова.'); }
...
// --- Bookings ---
const statusLabels = { pending:'Ожидает подтверждения', cancellation_pending:'Ожидает подтверждения отмены', reschedule_pending:'Ожидает подтверждения переноса', planned:'Запланирована', confirmed:'Подтверждена', in_progress:'Клиент пришёл', completed:'Завершена', cancelled:'Отменена', no_show:'Не явился', conflict:'Конфликт', disputed:'Спорная' };
...
async function loadBookings() {
    ...
    if (currentBookingTab === 'active') {
        params.set('status', 'planned,confirmed,in_progress');
    ...
    params.set('page', currentBookingPage);
    params.set('page_size', BOOKINGS_PAGE_SIZE);
    const response = await apiGet(`/bookings?${params}`);
    if (!response) return;
    const offlineItems = Array.isArray(response) ? response : null;
    const totalOfflinePages = offlineItems ? Math.max(1, Math.ceil(offlineItems.length / BOOKINGS_PAGE_SIZE)) : 0;
    if (offlineItems) currentBookingPage = Math.min(currentBookingPage, totalOfflinePages);
    const data = offlineItems
        ? offlineItems.slice((currentBookingPage - 1) * BOOKINGS_PAGE_SIZE, currentBookingPage * BOOKINGS_PAGE_SIZE)
        : (response.items || []);
    const pagination = offlineItems
        ? { page: currentBookingPage, page_size: BOOKINGS_PAGE_SIZE, total: offlineItems.length, total_pages: totalOfflinePages }
        : response.pagination;
...
function renderBookingsPagination(pagination) {
    ...
    const buttons = [...pages]
        ...
        .sort((a, b) => a - b)
        .map(number => `<button class="btn btn-sm ${number === page ? 'btn-primary' : 'btn-outline'}" onclick="changeBookingPage(${number})" ${number === page ? 'aria-current="page"' : ''}>${number}</button>`)
        .join('');
    container.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap;padding:16px 0 4px">
            <button class="btn btn-outline btn-sm" onclick="changeBookingPage(1)" ${page === 1 ? 'disabled' : ''} title="Первая страница">«</button>
            <button class="btn btn-outline btn-sm" onclick="changeBookingPage(${page - 1})" ${page === 1 ? 'disabled' : ''} title="Предыдущая страница">‹</button>
            ${buttons}
            <button class="btn btn-outline btn-sm" onclick="changeBookingPage(${page + 1})" ${page === totalPages ? 'disabled' : ''} title="Следующая страница">›</button>
            <button class="btn btn-outline btn-sm" onclick="changeBookingPage(${totalPages})" ${page === totalPages ? 'disabled' : ''} title="Последняя страница">»</button>
            <span style="font-size:12px;color:var(--text-secondary);margin-left:4px">Всего: ${total}</span>
        </div>`;
}
...
function paymentSummary(booking) {
    ...
    if (booking.referral_discount_amount > 0) {
        return `<strong>🎁 Скидка: ${booking.referral_discount_amount.toLocaleString()} ₸</strong><br><strong>К оплате: ${booking.price.toLocaleString()} ₸</strong>${booking.payment_status === 'paid' ? '<br><small style="color:var(--success)">✅ Оплата получена</small>' : ''}`;
    }
    return `<strong>${booking.price.toLocaleString()} ₸</strong>${booking.payment_status === 'paid' ? '<br><small style="color:var(--success)">✅ Оплата получена</small>' : ''}`;
}
...
const remindedBookingMinutes = new Set();
function updateBookingCountdowns() {
    const now = new Date();
    document.querySelectorAll('.booking-reminder-row[data-booking-start]').forEach(row => {
        const start = new Date(row.dataset.bookingStart);
        const minutes = Math.ceil((start - now) / 60000);
        const target = row.querySelector('.booking-countdown');
        if (target) target.textContent = minutes > 0 ? `через ${minutes} мин.` : 'началось / прошло';
        // Five short, deliberate attention windows: 65, 64, 63, 62 and 61
        // minutes before the lesson.  Each window blinks ten times then stops.
        const reminderKey = `${row.dataset.bookingStart}:${minutes}`;
        if (minutes >= 61 && minutes <= 65 && !remindedBookingMinutes.has(reminderKey)) {
            remindedBookingMinutes.add(reminderKey);
            let count = 0;
            const blink = setInterval(() => {
                row.classList.toggle('booking-reminder-flash');
                if (++count >= 20) { clearInterval(blink); row.classList.remove('booking-reminder-flash'); }
            }, 250);
        }
    });
}
setInterval(updateBookingCountdowns, 10000);
...
async function saveDailySchedule() {
    ...
    fillDailyScheduleForm(scheduleDate);
        ...
        // The API returns every affected booking number. Do not update local
        // state when it refuses a day off, otherwise the calendar lies.
        showToast(e.message || 'Не удалось сохранить расписание дня', 'error');
    }
}
    ...
    list.innerHTML = fleetVehicles.map(vehicle => `
        <article class="fleet-card" id="vehicle-card-${vehicle.id}">
            <div class="fleet-card-header">
                <div><span class="fleet-icon">🚗</span><strong>${escapeHtml(vehicle.name)}</strong></div>
                <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;justify-content:flex-end;">
                    <span class="fleet-transmission fleet-${vehicle.transmission}">${vehicleTransmissionLabel(vehicle.transmission)}</span>
                    ${vehicle.is_under_repair ? '<span class="badge badge-cancelled">На ремонте</span>' : ''}
                </div>
            </div>
            <div class="fleet-card-actions">
                <button class="btn btn-outline btn-sm" onclick="editVehicle(${vehicle.id})">Редактировать</button>
                <button class="btn ${vehicle.is_under_repair ? 'btn-primary' : 'btn-outline-danger'} btn-sm" onclick="toggleVehicleRepair(${vehicle.id}, ${!vehicle.is_under_repair})">${vehicle.is_under_repair ? 'Вернуть в строй' : 'На ремонте'}</button>
                <button class="btn btn-outline-danger btn-sm" onclick="deleteVehicle(${vehicle.id})">Удалить</button>
            </div>
...
function editVehicle(id) {
    document.getElementById(`vehicle-edit-${id}`)?.classList.toggle('hidden');
}
...
async function saveVehicle(id) {
    ...
    try {
        await apiPut(`/vehicles/${id}`, { name, transmission });
        showToast('Карточка машины обновлена');
        await loadVehicles();
    } catch (error) {
        showToast(error.message || 'Не удалось сохранить машину', 'error');
    }
}
...
async function addVehicle() {
    ...
    try {
        await apiPost('/vehicles', { name: name.trim(), transmission });
        showToast('Машина добавлена');
        await loadVehicles();
    } catch (error) {
        showToast(error.message || 'Не удалось добавить машину', 'error');
    }
}
...
async function deleteVehicle(id) {
    ...
    try {
        ...
        showToast(error.message || 'Нельзя удалить машину с активными записями', 'error');
    }
}
...
async function toggleVehicleRepair(id, isUnderRepair) {
    ...
    try {
        await apiPut(`/vehicles/${id}/repair`, { is_under_repair: isUnderRepair });
        showToast(isUnderRepair ? 'Машина снята со свободных слотов' : 'Машина возвращена в доступные слоты');
        await loadVehicles();
    } catch (error) {
        showToast(error.message || 'Не удалось изменить статус машины', 'error');
    }
}
...
async function saveInstructor() {
    ...
    if (instructorSaveInProgress) return;
    instructorSaveInProgress = true;
    try {
        ...
        instructorSaveInProgress = false;
    }
}
...
function renderRevenueAnalytics() {
    ...
    if (updated) {
        const refreshHours = Number(analyticsRevenueData?.refresh_interval_hours || 3);
        updated.textContent = analyticsRevenueData?.updated_at
            ? `Обновлено ${new Date(analyticsRevenueData.updated_at).toLocaleString('ru-RU', { timeZone: 'Asia/Almaty' })} · шкала ${periodData?.granularity_label || 'по интервалам'} · автообновление ${refreshHours} ч.`
    ...
    const grid = Array.from({ length: 5 }, (_, index) => {
        ...
        return `<line class="analytics-chart-grid" x1="${padding.left}" y1="${y}" x2="${width - padding.right}" y2="${y}"></line>
          <text class="analytics-chart-label" x="${width - padding.right + 10}" y="${y + 4}">${escapeHtml(formatRevenueAxis(value))}</text>`;
    }).join('');
    ...
    const xLabels = labelIndexes.map(index => `
        <line class="analytics-chart-grid analytics-chart-grid-vertical" x1="${xAt(index)}" y1="${padding.top}" x2="${xAt(index)}" y2="${volumeBottom}"></line>
        <text class="analytics-chart-label" x="${xAt(index)}" y="${height - 12}" text-anchor="middle">${escapeHtml(revenuePointLabel(points[index].timestamp, analyticsRevenuePeriod))}</text>`).join('');
    ...
    chart.innerHTML = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="График выручки за ${escapeHtml(periodData.label || '')}">
        <defs><linearGradient id="analyticsRevenueGradient" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stop-color="#64748b" stop-opacity=".16"></stop><stop offset="100%" stop-color="#64748b" stop-opacity=".01"></stop></linearGradient></defs>
        ${grid}${xLabels}<path class="analytics-chart-area" d="${areaPath}"></path><path class="analytics-chart-line" d="${linePath}"></path>${markers}${volumeBars}
      </svg>`;
}
...
async function loadAnalytics() {
    ...
    const renderSource = (key, data, period = 'all') => {
        const hasData = data && typeof data === 'object';
        const count = Number(data?.count || 0);
        const percent = Number(data?.percent || 0);
        const suffix = period === 'all' ? key : `${key}-${period}`;
        const countEl = document.getElementById(`analytics-source-${suffix}`);
        const percentEl = document.getElementById(`analytics-source-${suffix}-pct`);
        const barEl = document.getElementById(`analytics-source-${suffix}-bar`);
        if (countEl) countEl.textContent = hasData ? `${percent.toLocaleString('ru-RU')}%` : '—';
        if (percentEl) percentEl.textContent = hasData ? `${count.toLocaleString('ru-RU')} записей` : 'Нет данных';
        if (barEl) barEl.style.width = hasData ? `${Math.min(100, Math.max(0, percent))}%` : '0%';
    };
    const allSourceData = sourceData?.periods?.all;
    const todaySourceData = sourceData?.periods?.today;
    renderSource('telegram', todaySourceData?.telegram, 'today');
    renderSource('mobile', todaySourceData?.mobile, 'today');
    renderSource('manual', todaySourceData?.manual, 'today');
    renderSource('telegram', allSourceData?.telegram);
    renderSource('mobile', allSourceData?.mobile);
    renderSource('manual', allSourceData?.manual);
    const sourceTotalToday = document.getElementById('analytics-sources-total-today');
    const sourceTotalAll = document.getElementById('analytics-sources-total-all');
    if (sourceTotalToday) sourceTotalToday.textContent = todaySourceData
        ? `${Number(todaySourceData.total || 0).toLocaleString('ru-RU')} записей`
        : 'Нет данных';
    if (sourceTotalAll) sourceTotalAll.textContent = allSourceData
        ? `${Number(allSourceData.total || 0).toLocaleString('ru-RU')} записей`
        : 'Нет данных';

    const clientLessons = sourceData?.client_lessons;
    const clientLessonsAvailable = clientLessons && typeof clientLessons === 'object';
    const averageEl = document.getElementById('analytics-client-lessons-average');
    const averageDetailEl = document.getElementById('analytics-client-lessons-average-detail');
    const maximumEl = document.getElementById('analytics-client-lessons-maximum');
    const completedTotalEl = document.getElementById('analytics-client-lessons-total');
    if (averageEl) averageEl.textContent = clientLessonsAvailable
        ? Number(clientLessons.average_per_client || 0).toLocaleString('ru-RU', { maximumFractionDigits: 1 })
        : '—';
    if (averageDetailEl) averageDetailEl.textContent = clientLessonsAvailable
        ? `${Number(clientLessons.clients_counted || 0).toLocaleString('ru-RU')} клиентов учтено`
        : 'Нет данных';
    if (maximumEl) maximumEl.textContent = clientLessonsAvailable
        ? Number(clientLessons.maximum_per_client || 0).toLocaleString('ru-RU')
        : '—';
    if (completedTotalEl) completedTotalEl.textContent = clientLessonsAvailable
        ? `${Number(clientLessons.completed_lessons || 0).toLocaleString('ru-RU')} завершённых занятий`
        : 'Нет данных';
        ...
        if (pkgs && pkgs.length) {
            packagesHtml = `<div class="card" style="margin-bottom:20px">
                ...
                <div id="packages-list">${pkgs.map(p => `<div class="inst-card package-card">
                    <div class="inst-info">
                        <h3>${escapeHtml(p.name)}${p.code ? ` · <code>${escapeHtml(p.code)}</code>` : ''}</h3>
...
async function loadCertificateRequests(container) {
    ...
    updateNavBadge('packages', items.length);
...
async function createPackage(sessionsCount = 6) {
    const offers = {
        6: {
            name: 'Пакет 6 занятий', sessions_count: 6, price: 55000,
            validity_days: 30, bonus_exam: true,
            description: '6 занятий на площадке, пробный экзамен в подарок.'
        },
        10: {
            name: 'Пакет 10 занятий', sessions_count: 10, price: 90000,
            validity_days: 30, bonus_exam: true,
            description: '10 занятий на площадке, пробный экзамен в подарок.'
        }
    };
    const offer = offers[sessionsCount];
    if (!offer) {
        showToast('Неизвестный пакет', 'error');
        return;
    }
    try {
        const result = await apiPost('/packages', offer);
        const resultEl = document.getElementById('package-result');
        if (resultEl) resultEl.innerHTML = `✅ Пакет выпущен. Код: <strong>${escapeHtml(result.code || '')}</strong>`;
        showToast('Пакет выпущен');
        loadPackages();
    } catch (e) { showToast(e.message || 'Не удалось выпустить пакет', 'error'); }
}
...
async function createCertificate() {
    ...
    if (![3500, 5000, 10000].includes(nominal)) { showToast('Сертификат может быть на 3500, 5000 или 10000 ₸', 'error'); return; }
    try {
...
async function loadAudit() {
    ...
    tbody.innerHTML = data.map(l => `<tr>
        ...
        <td>${escapeHtml(l.action || 'Системное действие')}</td>
        <td><strong>${escapeHtml(l.admin_username || 'Система')}</strong></td>
    ...
    if (mobileCards) {
        mobileCards.innerHTML = data.map(l => `
            <div class="mobile-card">
                <div class="mobile-card-header">
                    <div class="mobile-card-title">${escapeHtml(l.action || 'Системное действие')}</div>
                ...
                <div class="mobile-card-row">
                    ...
                    <span class="mobile-card-value"><strong>${escapeHtml(l.admin_username || 'Система')}</strong></span>
                </div>
                ${l.details ? `
                <div class="mobile-card-row">
...
async function loadSettings() {
    const data = await apiGet('/settings/exam-sensors');
    const input = document.getElementById('exam-sensor-capacity');
    if (input && data) input.value = data.capacity;
}

async function saveExamSensorCapacity() {
    const input = document.getElementById('exam-sensor-capacity');
    const capacity = Number(input?.value);
    if (!Number.isInteger(capacity) || capacity < 0 || capacity > 99) {
        showToast('Укажите целое количество от 0 до 99', 'error');
        return;
    }
    const result = await apiPut('/settings/exam-sensors', { capacity });
    input.value = result.capacity;
    showToast('Количество комплектов датчиков сохранено');
}
...
async function selectClient(clientId) {
    ...
    if (!client) return;
    // Older data may contain accidental repeated assignments of the same
    // issued package. Show one package badge; new assignments are blocked by
    // the server and therefore cannot create such duplicates.
    const displayedPackages = Array.from(new Map(
        (client.packages || []).map(pkg => [pkg.package_id || pkg.id, pkg])
    ).values());
    ...
    card.innerHTML = `
        ...
        <div class="client-mini-details">
            ...
            ${displayedPackages.length > 0 ? `
                <div class="client-mini-row"><span class="client-mini-label">Пакеты:</span><span>${displayedPackages.map(pkg =>
        ...
        <div class="client-mini-actions">
            ...
            ${client.telegram_id
                ? `<button class="btn btn-primary btn-sm" onclick="openClientChat(${client.id})" title="Написать в клиентский Telegram-бот">💬 Написать <span style="font-size:10px">TG</span></button>`
                : `<button class="btn btn-outline btn-sm" onclick="showToast('У клиента нет Telegram-профиля: свяжитесь с ним по телефону или в WhatsApp', 'error')" title="Telegram-профиль не подключён">✉️ Связаться</button>`}
...
async function editClient(id) {
    try {
        ...
        // Packages are loaded here (not in the history dialog) because the
        // editor needs them to render its selector.
        const packages = await apiGet('/packages') || [];
        const assignedPackages = client.packages || [];
        const activatedCertificates = client.certificates || [];
        const assignedPackagesHtml = assignedPackages.length
            ? assignedPackages.map(p => `<div style="padding:8px 10px;margin-top:5px;background:var(--success-bg);color:var(--success);border-radius:6px;font-size:13px">✓ ${escapeHtml(p.name)}${p.code ? ` · ${escapeHtml(p.code)}` : ''} · осталось занятий: ${p.remaining_sessions}${p.remaining_bonus_exams ? ` · экзамен: ${p.remaining_bonus_exams}` : ''}</div>`).join('')
            : '<small style="color:var(--text-secondary)">Нет ранее назначенных пакетов.</small>';
        const certificatesHtml = activatedCertificates.length
            ? activatedCertificates.map(c => `<div style="padding:8px 10px;margin-top:5px;background:var(--success-bg);color:var(--success);border-radius:6px;font-size:13px">✓ ${escapeHtml(c.code)} · остаток: ${c.remaining} ₸${c.is_used ? ' · использован' : ''}</div>`).join('')
            : '<small style="color:var(--text-secondary)">Нет активированных сертификатов.</small>';
        ...
        document.getElementById('modal-body').innerHTML = `
            ...
            <div class="form-group">
                <label>Уже назначенные пакеты</label>
                ${assignedPackagesHtml}
            </div>
            <div class="form-group">
                <label>Выдать новый пакет</label>
                <select id="edit-client-package"><option value="">Не назначать</option>${packages.filter(p => p.is_available).map(p => `<option value="${p.id}">${escapeHtml(p.name)} · ${p.code || 'без кода'} · ${p.price.toLocaleString()} ₸</option>`).join('')}</select>
                <small style="color:var(--text-secondary);font-size:12px;margin-top:4px;display:block">Выданный пакет исчезает из списка и не может быть назначен повторно.</small>
            </div>
            <div class="form-group">
                <label>Уже активированные сертификаты</label>
                ${certificatesHtml}
            </div>
            <div class="form-group">
                <label>Активировать сертификат</label>
                <input type="text" id="edit-client-certificate" placeholder="Введите код сертификата для активации">
                <small style="color:var(--text-secondary);font-size:12px;margin-top:4px;display:block">Код будет активирован для клиента сразу после сохранения — подтверждение не требуется.</small>
            </div>
        `;
        document.getElementById('modal-save-btn').style.display = 'block';
        document.getElementById('modal-save-btn').onclick = async () => {
            ...
            try {
                ...
                const packageId = document.getElementById('edit-client-package').value;
                if (packageId) await apiPost('/clients/assign-package', { client_id: id, package_id: Number(packageId) });
                const certificateCode = document.getElementById('edit-client-certificate').value.trim();
                if (certificateCode) {
                    await apiPost('/clients/activate-certificate', {
                        client_id: id,
                        certificate_code: certificateCode,
                    });
                }
                showToast(packageId || certificateCode ? 'Клиент обновлён, пакет/сертификат применён' : 'Клиент обновлён');
                closeModal();
                loadClients();
            } catch (e) {
                showToast(e.message, 'error');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const res = await fetchWithTimeout(`${API}/check-session`, {
            credentials: 'include',
            cache: 'no-store',
        });
        const data = res.ok ? await res.json().catch(() => null) : null;
...
async function showManualBookingForm() {
    ...
    try {
        // The server calculates this in the Pavlodar timezone and shifts the
        // range only after the working day ends.
        const window = await apiGet('/booking-window');
...
async function loadSlots() {
    ...
    panel.innerHTML = header + '<div class="slots-list">' + data.slots.map(slot => {
        ...
        const occupiedCount = Number(slot.booked_count || slot.bookings.length || 0);
        const statusText = slot.is_free
            ? (occupiedCount ? `Есть место · занято ${occupiedCount}/${slot.capacity || 6}` : `Свободно · 0/${slot.capacity || 6}`)
...
function handleServiceChange() {
    ...
    if (service === 'exam') {
        locationSelect.innerHTML = '<option value="Циолковского 30">Циолковского 30 (3500₸)</option>';
...
window.saveExamSensorCapacity = saveExamSensorCapacity;
...
async function loadWaitingList() {
    ...
    // A client who requested a future date needs a call as soon as that date
    // arrives, even when no cancelled slot triggered the usual matcher.
    (data.items || []).filter(item => item.requires_attention).forEach(item => matchingIds.add(item.id));
...
function updateWaitingAttention(hasMatches) {
    ...
    tab.title = pulse ? 'Есть клиенты, которым требуется звонок или подходит свободный слот' : '';
}
...
async function checkPendingConflicts() {
    showToast('Проверяю офлайн-конфликты...');
    const result = await apiPost('/bookings/check-pending-conflicts', {});
    if (!result) return;
    if (result.duplicates_count > 0) {
        showToast(`⚠️ ${result.duplicates_count} возможных дублей отправлены на рассмотрение`, 'warning');
    }
...
async function loadConflicts() {
    ...
    for (const group of groups) {
        ...
        for (const b of group.bookings) {
            const isManual = b.source === 'manual' || b.source === 'admin' || b.source === 'admin_offline';
