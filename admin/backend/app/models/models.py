...
from sqlalchemy import (
    ...
    CheckConstraint, ForeignKey, Text, UniqueConstraint, text
)
...
def now_kz():
    """Возвращает текущее время в таймзоне Казахстана (без timezone info для БД)"""
...
class Instructor(Base):
    ...
    # training | exam | both. Keep old instructor cards compatible by default.
    ...
    days_off = Column(String(200), default="Суббота,Воскресенье")  # Оставляем для обратной совместимости
    # Stable client operation key.  It makes retries after a lost HTTP
    # response return the already created instructor instead of adding a copy.
...
class Vehicle(Base):
    """A physical car assigned to one active booking at a time."""
...
class BookingResource(Base):
    """Configurable capacity shared by every booking channel."""
    __tablename__ = "booking_resources"

    key = Column(String(50), primary_key=True)
    capacity = Column(Integer, default=1, server_default="1", nullable=False)

    __table_args__ = (
        CheckConstraint("capacity >= 0", name="ck_booking_resources_capacity_nonnegative"),
    )


class InstructorDayOff(Base):
    """Конкретные даты выходных дней инструктора"""
...
class Client(Base):
    ...
    # See Instructor.offline_operation_id.  Phone is optional, so it cannot
    # by itself protect an offline creation from being replayed twice.
    offline_operation_id = Column(String(128), unique=True, nullable=True)
    created_at = Column(DateTime, default=now_kz)
...
class ArchivedLog(Base):
    """Неизменяемая история дневных журналов.

    Ссылок на рабочие таблицы здесь намеренно нет: запись в истории должна
    остаться доступной даже после удаления клиента, записи или инструктора.
    Тройка ``source_type`` / ``source_log_id`` / ``created_at`` не позволяет
    дважды перенести одну и ту же строку при одновременном запуске фонового
    задания и остаётся безопасной, если локальная БД повторно использует ID.
    """
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    source_type = Column(String(20), nullable=False)  # audit или event
...
class NotificationSent(Base):
    ...
    sent_at = Column(DateTime, default=now_kz)
...
class MobileUserPackage(Base):
    """Legacy mobile package rows still present in the shared database."""
...
class MobileAppReview(Base):
    """Reviews created by the site mobile API and stored in the shared DB."""
...
class SupportMessage(Base):
    ...
    sender = Column(String(20), nullable=False)  # "user" или "admin"
    text = Column(Text, nullable=False)
    is_read = Column(Boolean, default=False)
    is_admin_read = Column(Boolean, default=False)  # Добавлено для отслеживания прочитанных админом сообщений
    offline_operation_id = Column(String(128), unique=True, nullable=True)
    created_at = Column(DateTime, default=now_kz)
...
class AdminState(Base):
    ...
    id = Column(Integer, primary_key=True, default=1)
    notifications_viewed_at = Column(DateTime, nullable=True)
    clients_viewed_at = Column(DateTime, nullable=True)
    # Watermarks do not depend on a server clock.  They make the read state
    # reliable even if imported rows contain an incorrect/future created_at.
    notifications_viewed_id = Column(Integer, nullable=True)
    clients_viewed_id = Column(Integer, nullable=True)
