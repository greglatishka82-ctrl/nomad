...
class ExamSensorCapacityRequest(BaseModel):
    capacity: int
...
@router.get("/settings/exam-sensors")
async def get_exam_sensor_settings(
    request: Request, db: AsyncSession = Depends(get_db)
):
    _get_admin_username(request)
    resource = await db.get(BookingResource, "exam_sensor_kits")
    return {"capacity": resource.capacity if resource else 0}


@router.put("/settings/exam-sensors")
async def update_exam_sensor_settings(
    request: Request,
    body: ExamSensorCapacityRequest,
    db: AsyncSession = Depends(get_db),
):
    username = _get_admin_username(request)
    if body.capacity < 0 or body.capacity > 99:
        raise HTTPException(status_code=400, detail="Количество комплектов должно быть от 0 до 99")
    resource = (await db.execute(
        select(BookingResource).where(
            BookingResource.key == "exam_sensor_kits"
        ).with_for_update()
    )).scalar_one_or_none()
    if resource is None:
        resource = BookingResource(key="exam_sensor_kits", capacity=body.capacity)
        db.add(resource)
    else:
        resource.capacity = body.capacity
    await db.commit()
    await _audit(
        db, username, "update_exam_sensor_capacity",
        f"Комплектов датчиков для пробного экзамена: {body.capacity}",
    )
...
async def get_slots(
    ...
    mobile_bookings = mobile_result.scalars().all()
    vehicle_capacity = await get_vehicle_capacity(db, transmission_value)
    ...
    while current_minutes <= last_start_minutes:
        ...
        booked_vehicle_capacity = await count_booked_vehicle_capacity(
            db, target_date, slot_time, slot_end_time, transmission_value,
        )
        ...
        slots.append({
            ...
            "is_free": is_free,
            "booked_count": booked_vehicle_capacity,
...
async def create_manual_booking(
    ...
    if not await slot_has_capacity(db, booking_date, st, et, location, transmission, service.value):
        detail = (
            "На это время нет свободного комплекта датчиков или машины с АКПП"
            if service == ServiceType.EXAM else
            "На это время нет свободного места на площадке"
        )
    ...
    if not has_capacity:
        detail = (
            "На это время нет свободного комплекта датчиков или машины с АКПП"
            if service == ServiceType.EXAM else
            "На это время заняты все машины с выбранной КПП"
        )
...
async def resolve_reschedule_request(
    ...
    has_capacity = await slot_has_capacity(
        ...
        booking.service_type.value if hasattr(booking.service_type, "value") else str(booking.service_type),
    ...
    has_capacity = await reserve_vehicle_capacity(
    ...
    if not has_capacity:
        detail = (
            "На это время нет свободного комплекта датчиков или машины с АКПП"
            if str(getattr(booking.service_type, "value", booking.service_type)) == "exam" else
            "На это время заняты все машины с выбранной КПП"
        )
        raise HTTPException(status_code=409, detail=detail)
...
async def certificate_create(request: Request, body: CertificateCreate, db: AsyncSession = Depends(get_db)):
    ...
    if body.nominal not in (settings.PRICE_EXAM, 5000, settings.PRICE_TRAINING_NEW):
        raise HTTPException(status_code=400, detail="Certificate nominal must be 3500, 5000 or 10000")
...
async def audit_logs(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    ACTION_TRANSLATIONS = {
        ...
        "update_exam_sensor_capacity": "Изменено количество комплектов датчиков",
...
async def export_full_backup(request: Request, format: str = "html", db: AsyncSession = Depends(get_db)):
    ...
    exam_sensor_resource = await db.get(BookingResource, "exam_sensor_kits")
    backup_data["exam_sensor_capacity"] = (
        exam_sensor_resource.capacity if exam_sensor_resource else 0
    )
...
async def import_full_backup(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        exam_sensor_capacity = backup_data.get('exam_sensor_capacity', 1)
        if not isinstance(exam_sensor_capacity, int) or not 0 <= exam_sensor_capacity <= 99:
            raise HTTPException(
                status_code=400,
                detail="В резервной копии неверно указано количество комплектов датчиков",
            )
        exam_sensor_resource = await db.get(BookingResource, "exam_sensor_kits")
        if exam_sensor_resource is None:
            db.add(BookingResource(
                key="exam_sensor_kits", capacity=exam_sensor_capacity
            ))
        else:
            exam_sensor_resource.capacity = exam_sensor_capacity
