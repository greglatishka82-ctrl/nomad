...
def _ensure_pg_enums(conn):
    ...
    Uses DO $$ ... EXCEPTION WHEN duplicate_object so a pre-existing type
    ...
    for table_name, table in Base.metadata.tables.items():
        ...
        for col in table.columns:
            ...
            if col.server_default is not None:
                ...
                # SQLAlchemy keeps a literal server_default as a Python string.
                # Quote it here; expressions such as text("CURRENT_TIMESTAMP")
                # must remain SQL expressions.
...
async def _run_explicit_migrations():
    """Run explicit SQL migrations. Each statement is idempotent."""
    from sqlalchemy import text
    migrations = [
        ...
        # Fields used by approval, numbering and offline conflict handling.  These
        # must be explicit: older production databases already have `bookings`,
        # therefore create_all() alone cannot add them.
        ...
        "ALTER TABLE bookings ADD COLUMN IF NOT EXISTS admin_viewed BOOLEAN NOT NULL DEFAULT TRUE;",

        # Last-line protection against double-booking.  API handlers validate
        # availability too, but this PostgreSQL exclusion constraint also
        # protects concurrent requests and imports that write to `bookings`
        # directly.  [) permits one booking to begin exactly when another ends.
        ...
        # AdminState table for tracking notification read status
        "CREATE TABLE IF NOT EXISTS admin_state (id INTEGER PRIMARY KEY DEFAULT 1, notifications_viewed_at TIMESTAMP);",
        "INSERT INTO admin_state (id) SELECT 1 WHERE NOT EXISTS (SELECT 1 FROM admin_state WHERE id = 1);",
        "ALTER TABLE admin_state ADD COLUMN IF NOT EXISTS clients_viewed_at TIMESTAMP;",
        "ALTER TABLE admin_state ADD COLUMN IF NOT EXISTS clients_viewed_id INTEGER;",
        "ALTER TABLE admin_state ADD COLUMN IF NOT EXISTS notifications_viewed_id INTEGER;",
        
        # RatingRecord migrations
        "ALTER TABLE rating_records ALTER COLUMN vote TYPE VARCHAR(50) USING vote::VARCHAR;",
        
        # Data fix for legacy/imported rows: NULL means the old database did
        # not store the flag, not that the instructor was deactivated.
        ...
        # The site backend removes booking history after 60 days; related
        # audit/event records must remain usable and client profiles untouched.
        ...
        # Убираем пробелы, скобки, тире, заменяем 8 на 7, добавляем + если нет
        """
        DO $$
        BEGIN
...
async def init_db():
    ...
    # Step 4: explicit idempotent SQL migrations
    await _run_explicit_migrations()
    await _sync_mobile_users_to_clients()
    await _backfill_booking_numbers()
    await verify_database_ready()


async def _backfill_booking_numbers():
    """Give legacy rows a stable six-digit number exactly once.

    Old installations predate the number column.  We never renumber rows which
    already have a number: that would invalidate numbers clients have saved.
    """
    ...
    async with async_session() as db:
        ...
        rows = (await db.execute(
            select(Booking).where(
                Booking.booking_number.is_(None),
                Booking.status.in_(["planned", "confirmed", "in_progress", "completed", "no_show"]),
            ).order_by(
        ...
        if not rows:
            return
        existing = (await db.execute(select(Booking.booking_number).where(
            Booking.booking_number.isnot(None)
        ))).scalars().all()
        next_number = max((int(v) for v in existing if str(v).isdigit()), default=-1) + 1
        for booking in rows:
            booking.booking_number = f"{next_number:06d}"
            next_number += 1
        await db.commit()
        logger.info("Assigned booking numbers to %s legacy rows", len(rows))
