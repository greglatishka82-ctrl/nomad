...
_EARLY_ARRIVAL_WINDOW = timedelta(minutes=20)
_EARLY_COMPLETION_WINDOW = timedelta(minutes=20)
...
async def _ask_attendance_confirmation(callback: types.CallbackQuery, arrived: bool) -> None:
    ...
    async with async_session() as db:
        ...
        if booking.status not in ("planned", "confirmed"):
            ...
            return
        starts_at = _scheduled_start(booking)
        allowed_at = starts_at - _EARLY_ARRIVAL_WINDOW if arrived else starts_at
        if now_kz() < allowed_at:
            action_name = "приход" if arrived else "неявку"
            await callback.answer(
                f"Отметить {action_name} можно не раньше {allowed_at.strftime('%d.%m.%Y %H:%M')}",
                show_alert=True,
            )
            return
...
async def _confirm_client_arrived(callback: types.CallbackQuery) -> None:
    ...
    async with async_session() as db:
        ...
        if booking.status == "completed":
            ...
            return
        allowed_at = _scheduled_start(booking) - _EARLY_ARRIVAL_WINDOW
        if now_kz() < allowed_at:
            await callback.answer(
                f"Отметить приход можно не раньше {allowed_at.strftime('%d.%m.%Y %H:%M')}",
                show_alert=True,
            )
            return
...
async def lesson_done(callback: types.CallbackQuery):
    ...
    async with async_session() as db:
        ...
        completion_allowed_at = ends_at - _EARLY_COMPLETION_WINDOW
        if now_kz() < completion_allowed_at:
            await callback.answer(
                f"Завершить занятие можно не раньше {completion_allowed_at.strftime('%d.%m.%Y %H:%M')}",
                show_alert=True,
            )
            return
