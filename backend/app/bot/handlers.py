...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if client:
        ...
        welcome_text = (
            ...
            f"📋 <b>Пробный экзамен — {settings.PRICE_EXAM // 1000} {settings.PRICE_EXAM % 1000:03d} ₸</b>\n"
...
async def _finalize_booking(message: Message, state: FSMContext, telegram_id: str, client: Client = None):
    ...
    async with async_session() as db:
        ...
        price = settings.PRICE_TRAINING_NEW if service_type == ServiceType.TRAINING else settings.PRICE_EXAM
        ...
        has_capacity = await reserve_vehicle_capacity(
            db, booking_date, start_t, end_t, transmission, service_type
        )
        if not has_capacity:
            await db.rollback()
            await message.answer(
