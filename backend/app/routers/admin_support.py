"""
Чат поддержки — сторона администратора.
GET  /api/admin/support/dialogs
GET  /api/admin/support/dialogs/{user_id}
POST /api/admin/support/dialogs/{user_id}/reply

Защищён сессионной аутентификацией администратора (та же что в роутере admin.py).
"""
...
from app.models.models import SupportMessage, MobileUser, MobileBooking, BookingStatus

router = APIRouter(prefix="/api/admin/support", tags=["admin-support"])


def _require_admin(request: Request) -> None:
    """Та же проверка сессии что используется в admin.py."""
...
async def list_dialogs(
    ...
    # Все пользователи у которых есть сообщения поддержки
    ...
    for row in rows:
        ...
        # Кол-во непрочитанных от пользователя
...
async def get_dialog(
    ...
    # Записи пользователя (краткая сводка)
...
async def reply_to_user(
    ...
    # TODO: FCM push-уведомление клиенту если есть fcm_token
    # if user.fcm_token:
    #     await send_push(user.fcm_token, "Поддержка NOMAD", text)
