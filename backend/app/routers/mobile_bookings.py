...
async def create_booking(
    ...
    has_capacity = await reserve_vehicle_capacity(
        db, body.booking_date, start_time_obj, end_time_obj, transmission_enum, service_type_enum
    )
    if not has_capacity:
        detail = (
            "На это время нет свободного комплекта датчиков или машины с АКПП. "
            "Обновите список и выберите другое время"
            if service_type_enum == ServiceType.EXAM else
            "На это время заняты все машины с выбранной КПП. Обновите список и выберите другое время"
        )
