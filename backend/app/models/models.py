...
class MobileUser(Base):
    """Пользователи мобильного приложения (независимы от Telegram-клиентов)."""
    ...
    referred_by = relationship("MobileUser", remote_side=[id],
                               foreign_keys=[referred_by_id])
    packages = relationship("MobileUserPackage", back_populates="user")
    certificates = relationship("MobileUserCertificate", back_populates="user")
...
class MobileBooking(Base):
    ...
    status = Column(Enum(BookingStatus), default=BookingStatus.PLANNED)
    price = Column(Integer, nullable=False)
    package_usage_id = Column(Integer, ForeignKey("mobile_user_packages.id"), nullable=True)
    certificate_id = Column(Integer, ForeignKey("mobile_user_certificates.id"), nullable=True)
    ...
    package_usage = relationship("MobileUserPackage", foreign_keys=[package_usage_id])
    certificate = relationship("MobileUserCertificate", foreign_keys=[certificate_id])


class MobileUserPackage(Base):
    """Пакет занятий, привязанный к пользователю мобильного приложения."""
    ...
    package_id = Column(Integer, ForeignKey("packages.id"), nullable=False)
    remaining_sessions = Column(Integer, nullable=False)
    is_active = Column(Boolean, default=False)  # активируется администратором
    purchased_at = Column(DateTime, default=datetime.utcnow)
    activated_at = Column(DateTime, nullable=True)

    user = relationship("MobileUser", back_populates="packages")
    package = relationship("Package")


class MobileUserCertificate(Base):
    """Сертификат, активированный пользователем мобильного приложения."""
    __tablename__ = "mobile_user_certificates"
    ...
    user = relationship("MobileUser", back_populates="certificates")
    certificate = relationship("Certificate")


class MobileReferralRecord(Base):
    """Реферальные записи для мобильных пользователей."""
    __tablename__ = "mobile_referral_records"

    id = Column(Integer, primary_key=True, autoincrement=True)
    referrer_id = Column(Integer, ForeignKey("mobile_users.id"), nullable=False)
    referred_id = Column(Integer, ForeignKey("mobile_users.id"), nullable=False)
    discount_applied = Column(Boolean, default=False)
    created_at = Column(DateTime, default=datetime.utcnow)


class SupportMessage(Base):
    """Сообщения чата поддержки между пользователем и администратором."""
    __tablename__ = "support_messages"

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("mobile_users.id"), nullable=False)
    sender = Column(String(10), nullable=False)  # 'user' или 'admin'
